...
n = 1000
iter <- 10^5
W <- generate_data(n = n, block = FALSE)
# ##############
# KL
# ##############
...
# ############################################
# Continue KL for similar computation time.
# ############################################
A1_cut_history <- cutA1
min_A1_cut_history <- cutA1
start_time <- proc.time() - timeA1
i <- 2
while(timeA1[[3]] < timeA2[[3]]) {
  if(result1$max_gain > 1e-04) {
    result1 <- KL(W, result1$A)
  } else {
    result1 <- KL(W)
  }
  timeA1 <- proc.time() - start_time
  cutA1_new <- cut_size(W, result1$A)
  A1_cut_history <- c(A1_cut_history, cutA1_new)
  if(cutA1_new < min_A1_cut_history[i - 1]) {
    A1 <- result1$A
    cutA1 <- cutA1_new
    min_A1_cut_history <- c(min_A1_cut_history, cutA1)
  } else {
    min_A1_cut_history <- c(min_A1_cut_history, min_A1_cut_history[i - 1])
  }
  i <- i + 1
}
...
# ##############
start_time <- proc.time()
result3 <- SAA(W, E = seq(cutA1, cutA1*2, length.out = 300), 
               initial_t = 100, MAX_ITER = iter)
...
# ##############
start_time <- proc.time()
result4 <- SAMC(W, E = seq(cutA1, cutA1*2, length.out = 300),
                MAX_ITER = iter)
...
#
# #####################################################
png(paste("images/graph_min_cut_n", n, "_iter", iter, ".png", sep = ""), 
    2000, 2000, res = 500)
colors <- c("black", "blue", "orange", "gray")
plot((1:length(min_A1_cut_history))*iter/length(min_A1_cut_history), 
     min_A1_cut_history, type = "l",
     xlim = c(0, iter),
     ylim = c(min(cutA1, cutA2, cutA3, cutA4), max(result2$t_history)),
     ylab = "minimum cut size", xlab = "iteration number",
     col = colors[1], lty = 1, xaxt = "n")
axis(1, at = 0:2*iter/2)
if(length(A1_cut_history) == 1) {
  lines(c(0, iter), rep(cutA1, 2), lty = 1, col = colors[1])
}
lines((1:(result2$iterations/10))*10, result2$min_t_history,
      col = colors[2], lty = 2)
lines((1:(result3$iterations/10))*10, result3$min_t_history, 
      col = colors[3], lty = 3)
lines((1:(result4$iterations/10))*10, result4$min_t_history, 
      col = colors[4], lty = 4)
legend("topright", c("KL", "SA", "SAA", "SAMC"), cex = 0.8, col = colors, 
       lty = 1:4, lwd = 2, bty = "n")
dev.off()

png(paste("images/graph_cut_n", n, "_iter", iter, ".png", sep = ""), 
    2000, 2000, res = 500)
colors <- c("black", "blue", "orange", "gray")
plot((1:length(A1_cut_history))*iter/length(A1_cut_history), 
     A1_cut_history, type = "l",
     xlim = c(0, iter),
     ylim = c(0.99*min(cutA1, cutA2, cutA3, cutA4), max(result2$t_history)),
...
lines((1:(result2$iterations/10))*10, result2$t_history,
      col = colors[2], lty = 2)
...
png(paste("images/graph_vals_time_n", n, "_iter", iter, ".png", sep = ""), 
    400, 400, res = 100)
vals <- matrix(round(c(cutA1, cutA2, cutA3, cutA4), digits = 1), nrow = 4)
vals <- cbind(vals,
              matrix(round(c(timeA1[[3]], timeA2[[3]], timeA3[[3]], timeA4[[3]]), 
                           digits = 1), nrow = 4))
colnames(vals) <- c("Min cut", "Run time (s)")
rownames(vals) <- c("KL", "SA", "SAA", "SAMC")
grid.table(vals)
dev.off()

png(paste("images/graph_time_n", n, "_iter", iter, ".png", sep = ""), 
    400, 400, res = 100)
vals <- 
colnames(vals) <- "Run time (s)"
rownames(vals) <- c("KL", "SA", "SAA", "SAMC")
grid.table(vals)
dev.off()
